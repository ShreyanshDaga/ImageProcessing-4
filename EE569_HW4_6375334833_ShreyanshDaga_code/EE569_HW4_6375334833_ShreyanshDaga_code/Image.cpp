...
Image::Image(string ImageName, int Rows, int Cols, int ColorFormat)
{
	this->pImageDate = NULL;
	this->iRows = Rows;
	this->iCols = Cols;
...
Pixel* Image::GetPixel(int X, int Y, int Z)
{ Pixel *pxP;
...
Pixel* Image::GetPixel(double dX, double dY)
{
	...
	if(X1 == this->iRows - 1 || Y1 == this->iCols)
	{
		return this->GetPixel(X1,Y1);
	}
...
int Image::GetBW(int X, int Y)
{
	...
	{
		pxP = this->GetPixel(X, Y);
		iBW = 0.3*pxP->GetRed() + 0.11*pxP->GetBlue()+ 0.59*pxP->GetGreen();
	}
...
Image* Image::Convert2Bin(int iT)
{
	...
	int iPos = this->GetImageName().find('.');
	sprintf(szFileName,"%s_Bin.raw",this->GetImageName().substr(0,iPos).c_str());
	...
	for(int i = 0;i<iRows;i++)
	{
		for(int j = 0;j<iCols;j++)
		{
			...
			if(pxP->GetBW() > iT)
				imTemp->pImageDate[i][j] = (unsigned char)255;
			else
				imTemp->pImageDate[i][j] = (unsigned char)0;
			delete pxP;
		}
	}	

	cout<<"\n*Converted "<<this->szImageName.c_str()<<" into Binary "<<szFileName<<" image";
	return imTemp;
}
...
double** Image::GetDoubleArray(bool bIsNormalized)
{
	double **daTemp;
	int iRows = this->iRows;
	int iCols = this->iCols;

	daTemp = new double*[iRows];
	for(int i = 0;i<iRows;i++)
		daTemp[i] = new double[iCols];
	...
	for(int i = 0;i<iRows;i++)
	{
		for(int j = 0;j<iCols;j++)
		{
			...
			if(bIsNormalized)
				daTemp[i][j] = pxP->GetBW()/255.00;
			else
				daTemp[i][j] = pxP->GetBW();			
...
void Image::SetIntArray(int **piaImage)
{
	for(int i = 0;i<this->iRows;i++)
	{
		for(int j = 0;j<this->iCols;j++)
		{
			Pixel pxP(piaImage[i][j]);
			
			this->SetPixel(i,j,pxP);
		}
	}
}

void Image::SetDoubleArray(double **piaImage, bool bNormalised)
{
	Pixel pxP;
	...
	for(int i = 0;i<this->iRows;i++)
	{
		...
		{
			if(bNormalised)
				pxP.iBW = (piaImage[i][j]*255);
			else
				pxP.iBW = (piaImage[i][j]);
...
void Image::FillColor(unsigned char cCol)
{
	Pixel pxP(cCol,cCol,cCol);
